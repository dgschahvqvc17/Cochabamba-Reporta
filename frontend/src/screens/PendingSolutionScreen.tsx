/**
 * Pantalla: Incidentes verificados pendientes de solución (MVC - View).
 *
 * HU12 — Asignar incidente para solución (Encargado de solución).
 * Lista los incidentes en estado VERIFICADO con búsqueda y paginación
 * para que el encargado de solución seleccione uno y lo asigne a un
 * responsable del personal de solución.
 *
 * @format
 */
...
import { loadPendingSolution } from '../controllers/incidentController';
...
type PendingSolutionScreenProps = {
  onBack: () => void;
...
function PendingSolutionScreen({
  onBack,
  onOpenIncident,
}: PendingSolutionScreenProps) {
  ...
  const load = useCallback(
    async (targetPage: number, refreshing = false) => {
      ...
      const result = await loadPendingSolution({
        page: targetPage,
        limit: PAGE_SIZE,
  ...
  return (
    <View style={styles.flex}>
      <AdminImageHeader
        background={fondo3}
        title="Pendientes de solución"
        subtitle={
          isLoading
            ? 'Consultando incidentes…'
            : `${total} incidente${total !== 1 ? 's' : ''} verificado${
                total !== 1 ? 's' : ''
              } pendiente${total !== 1 ? 's' : ''} de solución`
        }
        badge="SOLUCIÓN"
        ...
        {errorMessage ? (
          ...
          <View style={styles.emptyBox}>
            ...
            <Text style={styles.emptyText}>
              {hasSearch
                ? 'No se encontraron incidentes verificados con esa búsqueda.'
                : 'No hay incidentes verificados pendientes de solución en este momento.'}
            </Text>
          </View>
        ) : (
          <>
                ...
                >
                  ...
                  <View style={styles.cardFoot}>
                    <View style={styles.footLink}>
                      <Icon name="person" size={14} color={Colors.accent} />
                      <Text style={styles.footLinkText}>Asignar para solución</Text>
                    </View>
...
export default PendingSolutionScreen;