/**
 * Pantalla: Incidentes asignados para verificación (MVC - View).
 *
 * HU11 — Verificar incidente (Personal de verificación).
 * Lista los incidentes que se le asignaron al verificador autenticado
 * (asignación VERIFICACION activa) con búsqueda y paginación, para
 * seleccionar uno y registrar la decisión de verificación.
 *
 * @format
 */
...
import { loadAssignedVerification } from '../controllers/incidentController';
...
type VerificationQueueScreenProps = {
  onBack: () => void;
...
function VerificationQueueScreen({
  ...
  const load = useCallback(
    async (targetPage: number, refreshing = false) => {
      ...
      const result = await loadAssignedVerification({
        page: targetPage,
        limit: PAGE_SIZE,
          ...
          <View style={styles.emptyBox}>
            ...
            <Text style={styles.emptyText}>
              {hasSearch
                ...
                : 'No tienes incidentes asignados para verificar en este momento.'}
            </Text>
          </View>
        ) : (
          <>
