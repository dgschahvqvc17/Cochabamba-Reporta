...
const incidentService = {
   ...
   * `extraFields` permite persistir campos extra del nuevo estado
   * (por ejemplo rejected_reason en HU11) sin alterar llamadas previas.
   */
  ...
  async verifyIncident(user, incidentId, payload) {
    ...
    if (!(await this.isVerificationAssignee(user, incident.id))) {
      throw buildError(
